# Guide : Extraire et importer le certificat ElasticSearch dans le JDK

Ce guide explique comment :
1. Créer un répertoire temporaire.
2. Extraire le certificat SSL d'ElasticSearch avec OpenSSL.
...
## ✅ 2. Extraire le certificat SSL d'ElasticSearch
...
### Explications :
- `openssl s_client -showcerts -connect localhost:9200` : se connecte au service ElasticSearch sur le port 9200 et affiche les certificats.
...
```bash
keytool -import -alias ElasticSearchcert -file "C:\temp\certifs.cer" -keystore "C:\Program Files\Java\jdk-22\lib\security\cacerts"
```

### Explications :
- `-alias ElasticSearchcert` : nom sous lequel le certificat sera stocké.
...
```bash
keytool -list -keystore "C:\Program Files\Java\jdk-22\lib\security\cacerts" | findstr ElasticSearchcert
```
